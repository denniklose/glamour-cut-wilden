...
export default function SiteFooter() {
  return (
    <footer className="site-footer">
      ...
      <div className="footer-brand"><Link className="wordmark" href="/"><span>{salonConfig.logo.text}</span><small>{salonConfig.logo.subtitle}</small></Link><p>{site.address[0]} · {site.address[1]}<br />Telefon: {site.phoneDisplay || "Kontakt folgt"}{site.email ? <><br />{site.email}</> : null}</p></div>
